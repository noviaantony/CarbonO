...
class CarbonTrackerService {
  ...
  async getUserTotalCarbonConsumption(userId, userToken) {
    ...
    const response = await axios.get(CARBON_TRACKER_API_URL + '/getUserTotalCarbonConsumption', { params: { userId: userId }, headers: { Authorization: userToken } });
  ...
  async postDishConsumed(dishId, userToken, userId) {
    ...
    const response = await axios.post(
      ...
      null,
      {
        params: { dishId: dishId, userId: userId },
